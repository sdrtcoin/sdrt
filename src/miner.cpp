...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in sdrt, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in SDRT
//
...
// ***TODO*** that part changed in sdrt, we are using a mix with old one here for now
void static SDRTMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("SDRTMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sdrt-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("SDRTMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("SDRTMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("SDRTMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("SDRTMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateSDRTs(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SDRTMiner, boost::cref(chainparams), boost::ref(connman)));
}
