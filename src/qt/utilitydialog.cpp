    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("SDRT Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About SDRT Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  sdrt-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_SDRT_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SDRTGUI::DEFAULT_UIPLATFORM));
        }
...
All the SDRT in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 SDRT, 0.1 SDRT, 1 SDRT and 10 SDRT -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("SDRT Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(SDRTGUI *window)
{
    if (!window)
        return nullptr;
