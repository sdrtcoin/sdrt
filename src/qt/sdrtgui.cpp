...
const std::string SDRTGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SDRTGUI::DEFAULT_WALLET = "~Default";

SDRTGUI::SDRTGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/sdrt_logo_horizontal");
...
void SDRTGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a SDRT address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and sdrt: URIs)"));
    ...
    aboutAction = new QAction(tr("&About SDRT"), this);
    aboutAction->setStatusTip(tr("Show information about SDRT"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for SDRT"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your SDRT addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified SDRT addresses"));
    ...
    openAction->setStatusTip(tr("Open a sdrt: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the SDRT help message to get a list with possible SDRT command-line options"));
...
void SDRTGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void SDRTGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool SDRTGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool SDRTGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SDRTGUI::setWalletActionsEnabled(bool enabled)
{
...
void SDRTGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("SDRT client") + " " + networkStyle->getTitleAddText();
...
void SDRTGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void SDRTGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SDRTGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SDRTGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SDRTGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void SDRTGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void SDRTGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void SDRTGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SDRTGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SDRTGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SDRTGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void SDRTGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to SDRT network", "", count));
    } else {
...
void SDRTGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SDRTGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void SDRTGUI::updateHeadersSyncProgressLabel()
{
...
void SDRTGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void SDRTGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void SDRTGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("SDRT"); // default title
...
void SDRTGUI::changeEvent(QEvent *e)
{
...
void SDRTGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SDRTGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SDRTGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SDRTUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void SDRTGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SDRTGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SDRTGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SDRTGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SDRTGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void SDRTGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void SDRTGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SDRTGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SDRTGUI::detectShutdown()
{
...
void SDRTGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SDRTGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void SDRTGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SDRTGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SDRTGUI::subscribeToCoreSignals()
{
...
void SDRTGUI::unsubscribeFromCoreSignals()
{
...
void SDRTGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void SDRTGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<SDRTUnits::Unit> units = SDRTUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const SDRTUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(SDRTUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(SDRTUnits::Unit u, SDRTUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(SDRTUnits::name(u)), this);
...
{
    setText(SDRTUnits::name(newUnits));
}
