...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "SDRTNetwork") type = SDRTNetwork;
    if(strType == "SDRTNetworkVariable") type = SDRTNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == SDRTNetwork) s = "SDRTNetwork";
    if(type == SDRTNetworkVariable) s = "SDRTNetworkVariable";
