...
const char * const SDRT_CONF_FILENAME = "sdrt.conf";
const char * const SDRT_PID_FILENAME = "sdrtd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sdrt" is a composite category enabling all SDRT-related debug output
            if(ptrCategory->count(string("sdrt"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SDRTCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SDRTCore
    // Mac: ~/Library/Application Support/SDRTCore
    // Unix: ~/.sdrtcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SDRTCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SDRTCore";
#else
    // Unix
    return pathRet / ".sdrtcore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", SDRT_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sdrt.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sdrt.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", SDRT_PID_FILENAME));
