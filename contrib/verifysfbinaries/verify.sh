...
###   This script attempts to download the signature file SHA256SUMS.asc from sdrt.org
...
RCSUBDIR="test/"
BASEDIR="https://sdrt.org/bin/"
VERSIONPREFIX="sdrt-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'sdrt-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the SDRT Core binary release signing key installed?"
   fi
